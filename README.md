...
사용자가 제공한 구성 그림을 바탕으로 정리한 초안입니다. 아래 선은 연결 관계를 표현하며, 정확한 데이터 방향과 통신 방식은 확인이 필요합니다.

```mermaid
flowchart LR
    U["실험 PC 1 · Ubuntu"] --- C["UR 컨트롤러"]
    C --- R["UR 로봇"]
    T["Teach Pendant"] --- C
    H["Helios"] --- U
    O["OptiTrack"] --- W["실험 PC 2 · Windows"]
    W --- U
    P["개인 개발 PC · 개발 및 시뮬레이션"] -. "연계 방식 확인 필요" .- U
    G["GitHub · 문서 및 코드 관리"] -.-> U
    G -.-> P
```
...
| 개인 개발 PC | 개발 및 시뮬레이션 환경 | 운영체제, 가상머신 사용 여부, 실험 PC 연계 방식 |
| GitHub | 설치 문서와 코드 관리 | 코드 저장 위치 및 관리 절차 |
